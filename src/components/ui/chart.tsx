    ...
    return (
        <ChartContext.Provider value={{ config }}>
            <div
                ...
                ref={ref}
                className={cn(
                    "flex aspect-video justify-center text-xs [&_.recharts-cartesian-axis-tick_text]:fill-[#64748B] [&_.recharts-cartesian-grid_line[stroke='#ccc']]:stroke-[#CBD5E1]/50 [&_.recharts-curve.recharts-tooltip-cursor]:stroke-[#CBD5E1] [&_.recharts-dot[stroke='#fff']]:stroke-transparent [&_.recharts-layer]:outline-none [&_.recharts-polar-grid_[stroke='#ccc']]:stroke-[#CBD5E1] [&_.recharts-radial-bar-background-sector]:fill-[#E2E8F0] [&_.recharts-rectangle.recharts-tooltip-cursor]:fill-[#E2E8F0] [&_.recharts-reference-line_[stroke='#ccc']]:stroke-[#CBD5E1] [&_.recharts-sector[stroke='#fff']]:stroke-transparent [&_.recharts-sector]:outline-none [&_.recharts-surface]:outline-none",
                    className,
                )}
                {...props}
            >
...
const ChartTooltipContent = React.forwardRef<HTMLDivElement, React.ComponentProps<typeof RechartsPrimitive.Tooltip> & React.ComponentProps<'div'> & { hideLabel?: boolean; hideIndicator?: boolean; indicator?: 'line' | 'dot' | 'dashed'; nameKey?: string; labelKey?: string }>(
    ({ active, payload, className, indicator = 'dot', hideLabel = false, hideIndicator = false, label, labelFormatter, labelClassName, formatter, color, nameKey, labelKey }, ref) => {
        ...
        return (
            <div
                ref={ref}
                className={cn('grid min-w-[8rem] items-start gap-1.5 rounded-lg border border-[#CBD5E1]/50 bg-[#FFFFFF] px-2.5 py-1.5 text-xs shadow-xl', className)}
            >
                ...
                <div className="grid gap-1.5">
                    {payload.map((item, index) => {
                        ...
                        return (
                            <div
                                key={item.dataKey}
                                className={cn('flex w-full flex-wrap items-stretch gap-2 [&>svg]:h-2.5 [&>svg]:w-2.5 [&>svg]:text-[#64748B]', indicator === 'dot' && 'items-center')}
                            >
                                {formatter && item?.value !== undefined && item.name ? (
                                    ...
                                    <>
                                        ...
                                        <div className={cn('flex flex-1 justify-between leading-none', nestLabel ? 'items-end' : 'items-center')}>
                                            <div className="grid gap-1.5">
                                                {nestLabel ? tooltipLabel : null} <span className="text-[#64748B]">{itemConfig?.label || item.name}</span>
                                            </div>
                                            {item.value && <span className="font-mono font-medium tabular-nums text-[#212B36]">{item.value.toLocaleString()}</span>}
                                        </div>
                                    </>
                                )}
                            </div>
