...
const DrawerContent = React.forwardRef<React.ElementRef<typeof DrawerPrimitive.Content>, React.ComponentPropsWithoutRef<typeof DrawerPrimitive.Content>>(({ className, children, ...props }, ref) => (
    <DrawerPortal>
        ...
        <DrawerPrimitive.Content
            ref={ref}
            className={cn('fixed inset-x-0 bottom-0 z-50 mt-24 flex h-auto flex-col rounded-t-[10px] border border-[#CBD5E1] bg-[#FFFFFF]', className)}
            {...props}
        >
            <div className="mx-auto mt-4 h-2 w-[100px] rounded-full bg-[#E2E8F0]" /> {children}
        </DrawerPrimitive.Content>
    </DrawerPortal>
));
